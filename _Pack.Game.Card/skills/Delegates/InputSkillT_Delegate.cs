...
namespace Pack
{
    public class InputSkill_Delegate:InputSkill
    {
        ...
        //forPreView
        public Func1<InputSkill, int, IEnumerable<string>> F_NodeForm { get; set; } = new Func1<InputSkill, int, IEnumerable<string>>().SetDef(null);
        public override IEnumerable<string> NodeForm(int kind) { return F_NodeForm.Invoke(kind); }

        //----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        public Func2<InputSkill,string, int, N<float>> F_GetFloat { get; set; } = new Func2<InputSkill, string, int, N<float>>().SetDef(null);
        public override N<float> GetFloat(string DataName, int kind) { return F_GetFloat.Invoke(DataName, kind); }

        public Func2<InputSkill, string, int, N<bool>> F_GetBool { get; set; }= new Func2<InputSkill, string,int, N<bool>>().SetDef(null);
        ...
        public Func1<InputSkill, InputNode, bool> F_TestNodeUseful { get; set; } = new Func1<InputSkill, InputNode, bool>().SetDef(true);
